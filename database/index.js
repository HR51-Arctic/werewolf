...
const registerUser = (username, password, email, callback) => {
  db.connect((err, client, release) => {
    if (err) {
      ...
      client.query(`INSERT INTO users (username, userPassword, email) VALUES ('${username}', '${password}', '${email}')`, (err, result) => {
        release();
        if (err) {
...
const verifyUser = (username, callback) => {
  db.connect((err, client, release) => {
    if (err) {
      ...
      client.query(`SELECT userpassword FROM users WHERE username = '${username}'`, (err, result) => {
        release();
        if (err) {
          return callback(err, null);
        } else {
